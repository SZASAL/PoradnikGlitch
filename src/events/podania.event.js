module.exports =  {
    ...
    async run(msg){
        ...
        if(listakanalow.join().includes(`${msg.channel.id}`) && listakanalow.join().includes(`${msg.channel.parent.id}`)){
            ...
            const embed = new MessageEmbed().setAuthor(namebot, avatar).setTitle(`Nowe podanie!`).setFooter(`${msg.author.id}`).setColor(`#ffe600`).setThumbnail(`${msg.author.avatarURL()}`).setDescription(`**Autor podania:** ${msg.author} | ${msg.author.tag}\n
